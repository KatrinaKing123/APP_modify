...
% 
% %% Driving cycle path
% Cycle_Name = Condition_name;
% % Cycle_Data_Path = strcat(Path.In ,Symbol,'+CycleData',Symbol,Cycle_Name,'.mat');
% Cycle_Data_Path = strcat(Path.In,'+CycleData',Symbol,Cycle_Name);
% 
% Data_Path.Cycle_Data_Path = Cycle_Data_Path;

%% Design vehicle dynamics model
Index_Start = 1;   
% Index_End = Condition_length;
Index_End = length(velocity);
% SOC_Init = 0.90;
run VehParameters.EV_Par.m
EV = Example.EV_3S_SJTU.Cal_EV_Model(velocity,Index_Start, Index_End,Para);
...
%% Initial state
Input_K = Example.EV_3S_SJTU.Init_Put_K_Struct();
Input_K.SOC_K_1_Grid = SOC_Init;
Input_K.J_K_1_Grid = 0;

Input_K.G_K_1_Grid = 0.5;
...
for Step = 1: EV.Cycle_Num - 1
    ...
    %% Main 
    Out_Put_K= Example.EV_3S_SJTU.DP_Main_Function(EV,SOC_K_1,Input_K,Step);
    
    %% Data merge
    OutPut_K = Example.EV_3S_SJTU.CS_Data_Merge(Out_Put_K);
