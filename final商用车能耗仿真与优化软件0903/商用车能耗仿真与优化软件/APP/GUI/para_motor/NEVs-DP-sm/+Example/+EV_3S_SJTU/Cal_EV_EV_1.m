...
if(min(S_Pb_K_Grid) >= 0)
    ...
    proportion_1 = S_P_em_K_Grid_Real./S_P_em_K_Grid;%电池功率若超出，前后电机功率按同一比例削减;因为有附加单元，要算电机功率
    S_P_em_Front_K_Grid = S_P_em_Front_K_Grid.*proportion_1;%需要逆推出充电时受到限制后的电机实际扭矩
    S_P_em_Rear_K_Grid = S_P_em_Rear_K_Grid.*proportion_1;%前面算电机扭矩已经将扭矩限制到电机最小扭矩（负）了，现在扭矩不会更小（负）

     %最大充电电流对于电机扭矩的限制
    EV.Batt.SOC_K = S_SOC_K_Grid;
    EV.Batt.P_Batt_K = S_Pb_K_Grid;
    EV.Batt.restrict_I();
    S_Pb_K_Grid_actual = EV.Batt.Pb_Real;
    S_P_em_K_Grid_actual = S_Pb_K_Grid_actual - EV.Paux;
    proportion_2 = S_P_em_K_Grid_actual./S_P_em_K_Grid_Real;%前后电机功率同样按同一比例削减
    S_P_em_Front_K_Grid = S_P_em_Front_K_Grid.*proportion_2;
    S_P_em_Rear_K_Grid = S_P_em_Rear_K_Grid.*proportion_2;
    ...
    S_Pb_K_Grid = S_Pb_K_Grid_actual;  
end
...
% %     Index_negative = ((S_T_M_Front_K_Grid_Real<=0)&(S_T_M_Rear_K_Grid_Real<=0));
%     Index_E = find(Index_positive|Index_negative);
    
    Index_E=(1:1:length(S_SOC_K_Grid));
