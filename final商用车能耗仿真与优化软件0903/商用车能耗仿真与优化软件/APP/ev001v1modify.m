...
% global houqu
%% 求电机相关参数
Pe=1/nt*(GVW_n*g*f*Vmax/3600+Cd*A*Vmax^3/76140);                           %最高车速确定电机额定功率

Pi0=1/(3600*tm*nt)*(delta*GVW_n*Vm^2/7.2+GVW_n*g*f*Vm/1.5*tm+Cd*A*Vm^3/21.15/2.5*tm); %百公里加速确定电机峰值功率
...
if qianqu ==1
    Tmax = (GVW * g * (Forward/Line*cos(atan(theta))+hg/Line * sin(atan(theta))) * f  + GVW * g *sin(atan(theta))) * r0 / nt / i0;
else
...
%% 动力电池选型
Pie=Pe/nt/n0+Pa;                                                           %电池额定功率
Pimax=Pmax/nt/n0+Pa;                                                       %电池峰值功率
Pimax=max(Pie,Pimax);
%% WLTP工况
